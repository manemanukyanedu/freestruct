<template>
    <div >
      ...
      <div class="btn-hld" slot="buttons">
          ...
          <regular-button v-bind="buttons.remove"  @remove="remove()" v-if="showRemoveButton"></regular-button>
...
export default {
  ...
  data () {
    return {
      ...
      buttons: {
        ...
        remove: {name: 'Remove', active: true, classes: [], eventName: 'remove'},
  ...
  methods: {
    ...
    selectView () {
      if (this.selectedView === 'CreateQuestions') {
    ...
    reset () {
      if (this.questionMode === '') {
        ...
        this.setValuesOfSelectedQuestion()
      }
    },
    getAnswers () {
      let a = []
    ...
    setValuesOfSelectedQuestion () {
      this.question.value = this.selectedQuestion.question
      this.answers = []
    ...
    init () {
      if (this.questionMode === 'edit') {
        this.setValuesOfSelectedQuestion()
      } else {
        for (let i = Constants.MINQUIESTIONCOUNT - 1; i >= 0; i--) {
          this.addAnswer()
        }
      }
    },
    extractTool (name, component) {
      this.setQuestionMode('')
  ...
  async beforeRouteLeave (to, from, next) {
    if (!this.saved && this.question.value.trim() !== '') {
      ...
      const answer = await this.promise
      if (answer) {
        this.extractTool('Edit Question', 'EditQuestion')
        next()
      } else {
